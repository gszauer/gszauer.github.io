...
export default class LevelManager {
    ...
    levelExists(levelId) {
        return LevelData.getLevel(levelId) !== undefined;
    }
    
    getRandomUnlockedSpell() {
        // Get unlocked spells from the upgrades manager
        if (!this.scene.upgradesManager) return null;
        
        const unlockedSpells = [];
        if (this.scene.upgradesManager.hasSpell('spell1')) {
            unlockedSpells.push('spell1');
        }
        if (this.scene.upgradesManager.hasSpell('spell2')) {
            unlockedSpells.push('spell2');
        }
        if (this.scene.upgradesManager.hasSpell('spell3')) {
            unlockedSpells.push('spell3');
        }
        
        if (unlockedSpells.length === 0) return null;
        
        // Return a random unlocked spell
        return unlockedSpells[Math.floor(Math.random() * unlockedSpells.length)];
    }
        ...
        for (let row = 0; row < this.gridDimensions.rows; row++) {
            ...
            for (let col = 0; col < this.gridDimensions.lanes; col++) {
                ...
                if (char === 'M') {
                    ...
                    // Only spawn powerup if player has at least one spell unlocked
                    const powerUpType = this.getRandomUnlockedSpell();
                    if (powerUpType) {
                        const powerUp = new PowerUp(this.scene, x, y, powerUpType);
                        entities.powerUps.push(powerUp);
                        this.spawnedEntities.push(powerUp);
                    }
                }
            }
        }
