...
export default class CollisionManager {
    ...
    checkCollisions(player, hammer, monsters, obstacles, exitDoor, powerUps = []) {
        ...
        if (hammer.isSwinging()) {
            ...
            this.checkHammerPowerUpCollisions(hammer, powerUps);
        }
        ...
        for (let i = monsters.length - 1; i >= 0; i--) {
            ...
            if (!monster.isBouncing()) {
                ...
                if (obbCircleCollision(hammerOBB, monsterBounds)) {
                    // If gold touch is active, instantly destroy monster with flash effect
                    if (this.scene.powerUpManager && this.scene.powerUpManager.isGoldTouchActive()) {
                        this.scene.powerUpManager.goldTouchKill(monster);
                    } else {
                        // Normal knockback behavior
    ...
    checkHammerPowerUpCollisions(hammer, powerUps) {
        const hammerOBB = hammer.getOBB();
        ...
        for (let i = powerUps.length - 1; i >= 0; i--) {
            ...
            if (!powerUp.collected) {
                ...
                if (obbCircleCollision(hammerOBB, powerUpBounds)) {
                    if (powerUp.collect()) {
                        // Activate the power-up
                        this.scene.powerUpManager.activatePowerUp(powerUp.type);
