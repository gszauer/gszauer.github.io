class FileLoader {
    constructor(wasmImportObject, memoryAllocator) {
        if (!wasmImportObject.hasOwnProperty("env")) {
            wasmImportObject.env = {};
        }
        let self = this;
        
        wasmImportObject.env["LoadFileAsynch"] = function(path, cached, target, bytes, callback, userData) {
            // TODO: Implement CachedFile function
            self.LoadFile(path, target, bytes, callback, userData)
        }
    ...
    AttachToWasmInstance(wasmInstance) {
        ...
        if (this.queuedCallbacks != null) {
            while (this.queuedCallbacks.length != 0) {
                ...
                this.LoadFile(callback.path, callback.target, callback.bytes, callback.callback, callback.userData);
            }
            this.queuedCallbacks = null;
        }
    }

    LoadFile(_path, _target, _bytes, _callback, _userData) {
        let self = this;
        if (!this.callbacksEnabled) {
            ...
            let stringPath = this.decoder.decode(new Uint8Array(this.mem_buffer, _path, iter - _path));
