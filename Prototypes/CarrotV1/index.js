...
var ASM_CONSTS = {
  2999552: ($0) => { console.error(UTF8ToString($0)); alert(UTF8ToString($0)); },  
 2999614: () => { return window.devicePixelRatio || 1.0; },  
 2999657: ($0, $1, $2, $3) => { const dataPtr = $0; const dataSize = $1; const callbackPtr = $2; const filenamePtr = $3; let filename = "export.dat"; if (filenamePtr !== 0) { filename = UTF8ToString(filenamePtr); } const memoryArray = new Uint8Array(Module.HEAPU8.buffer, dataPtr, dataSize); const blobObject = new Blob([memoryArray], { type: "application/octet-stream" }); const element = document.createElement('a'); element.setAttribute('href', window.URL.createObjectURL(blobObject)); element.setAttribute('download', filename); element.style.display = 'none'; document.body.appendChild(element); element.click(); document.body.removeChild(element); window.URL.revokeObjectURL(element.href); if (callbackPtr !== 0) { Module._TriggerSaveAsCallback(callbackPtr, 0); } },  
 3000399: ($0, $1) => { const filterPtr = $0; const callbackPtr = $1; let fileElement = document.getElementById("platformFileUploader"); if (!fileElement) { fileElement = document.createElement('input'); fileElement.type = "file"; fileElement.id = "platformFileUploader"; fileElement.style.display = 'none'; document.body.appendChild(fileElement); fileElement.addEventListener('change', function() { const callback = this.pendingCallback; this.pendingCallback = null; if (!callback || this.files.length === 0) { if (callback) { Module._TriggerSelectFileCallback(callback, 0, 0, 0); } this.value = ""; return; } const file = this.files[0]; file.arrayBuffer().then(arrayBuffer => { const uint8Array = new Uint8Array(arrayBuffer); const dataPtr = Module._AllocateMemory(uint8Array.length); const dataArray = new Uint8Array(Module.HEAPU8.buffer, dataPtr, uint8Array.length); for (let i = 0; i < uint8Array.length; i++) { dataArray[i] = uint8Array[i]; } let namePtr = 0; if (file.name) { const encoder = new TextEncoder(); const nameBytes = encoder.encode(file.name); namePtr = Module._AllocateMemory(nameBytes.length + 1); const nameArray = new Uint8Array(Module.HEAPU8.buffer, namePtr, nameBytes.length + 1); for (let i = 0; i < nameBytes.length; i++) { nameArray[i] = nameBytes[i]; } nameArray[nameBytes.length] = 0; } Module._TriggerSelectFileCallback(callback, namePtr, dataPtr, uint8Array.length); if (namePtr !== 0) { Module._FreeMemory(namePtr); } if (dataPtr !== 0) { Module._FreeMemory(dataPtr); } this.value = ""; }).catch(error => { console.error("Error reading file:", error); Module._TriggerSelectFileCallback(callback, 0, 0, 0); this.value = ""; }); }); } if (filterPtr !== 0) { const filterStr = UTF8ToString(filterPtr); if (filterStr && filterStr.length > 0) { fileElement.accept = filterStr; } } fileElement.pendingCallback = callbackPtr; fileElement.click(); },  
 3002252: ($0) => { const messagePtr = $0; const messageStr = messagePtr ? UTF8ToString(messagePtr) : "Are you sure?"; return confirm(messageStr) ? 1 : 0; },  
 3002391: ($0) => { const textPtr = $0; const textStr = UTF8ToString(textPtr); if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(textStr).catch(err => { console.error('Failed to write to clipboard:', err); }); } else { const textArea = document.createElement('textarea'); textArea.value = textStr; textArea.style.position = 'fixed'; textArea.style.left = '-999999px'; document.body.appendChild(textArea); textArea.select(); try { document.execCommand('copy'); } catch (err) { console.error('Failed to copy to clipboard:', err); } document.body.removeChild(textArea); } },  
 3002984: () => { const userInput = prompt("Paste your text here (Ctrl+V or Cmd+V):", ""); if (userInput === null || userInput === "") { return 0; } const len = lengthBytesUTF8(userInput) + 1; const ptr = Module._AllocateMemory(len); stringToUTF8(userInput, ptr, len); return ptr; },  
 3003251: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 3003398: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
 3003632: ($0) => { if(typeof(Module['SDL2']) === 'undefined') { Module['SDL2'] = {}; } var SDL2 = Module['SDL2']; if (!$0) { SDL2.audio = {}; } else { SDL2.capture = {}; } if (!SDL2.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL2.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL2.audioContext = new webkitAudioContext(); } if (SDL2.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL2.audioContext); } } } return SDL2.audioContext === undefined ? -1 : 0; },  
 3004184: () => { var SDL2 = Module['SDL2']; return SDL2.audioContext.sampleRate; },  
 3004252: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; var have_microphone = function(stream) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); SDL2.capture.silenceTimer = undefined; SDL2.capture.silenceBuffer = undefined } SDL2.capture.mediaStreamNode = SDL2.audioContext.createMediaStreamSource(stream); SDL2.capture.scriptProcessorNode = SDL2.audioContext.createScriptProcessor($1, $0, 1); SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL2 === undefined) || (SDL2.capture === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL2.capture.currentCaptureBuffer = audioProcessingEvent.inputBuffer; dynCall('vi', $2, [$3]); }; SDL2.capture.mediaStreamNode.connect(SDL2.capture.scriptProcessorNode); SDL2.capture.scriptProcessorNode.connect(SDL2.audioContext.destination); SDL2.capture.stream = stream; }; var no_microphone = function(error) { }; SDL2.capture.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.capture.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL2.capture.currentCaptureBuffer = SDL2.capture.silenceBuffer; dynCall('vi', $2, [$3]); }; SDL2.capture.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); } },  
 3005945: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; SDL2.audio.scriptProcessorNode = SDL2.audioContext['createScriptProcessor']($1, 0, $0); SDL2.audio.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL2 === undefined) || (SDL2.audio === undefined)) { return; } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); SDL2.audio.silenceTimer = undefined; SDL2.audio.silenceBuffer = undefined; } SDL2.audio.currentOutputBuffer = e['outputBuffer']; dynCall('vi', $2, [$3]); }; SDL2.audio.scriptProcessorNode['connect'](SDL2.audioContext['destination']); if (SDL2.audioContext.state === 'suspended') { SDL2.audio.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.audio.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { if ((typeof navigator.userActivation) !== 'undefined') { if (navigator.userActivation.hasBeenActive) { SDL2.audioContext.resume(); } } SDL2.audio.currentOutputBuffer = SDL2.audio.silenceBuffer; dynCall('vi', $2, [$3]); SDL2.audio.currentOutputBuffer = undefined; }; SDL2.audio.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); } },  
 ...
 3007725: ($0, $1) => { var SDL2 = Module['SDL2']; var buf = $0 >>> 2; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[buf + (j*numChannels + c)]; } } },  
 3008214: ($0) => { var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; } },  
 3009220: ($0, $1, $2) => { var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Module['createContext'](Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); },  
 3010688: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 3011676: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 3011759: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 3011828: () => { return window.innerWidth; },  
 3011858: () => { return window.innerHeight; }
};
function setupFileDrop() { if (!Module.fileDropSetup) { Module.fileDropSetup = true; var canvas = Module.canvas; var dropArea = canvas.parentElement || document.body; var events = ['dragenter', 'dragover', 'dragleave', 'drop']; for (var i = 0; i < events.length; i++) { dropArea.addEventListener(events[i], preventDefaults, false); } function preventDefaults(e) { e.preventDefault(); e.stopPropagation(); } var highlightEvents = ['dragenter', 'dragover']; for (var i = 0; i < highlightEvents.length; i++) { dropArea.addEventListener(highlightEvents[i], highlight, false); } var unhighlightEvents = ['dragleave', 'drop']; for (var i = 0; i < unhighlightEvents.length; i++) { dropArea.addEventListener(unhighlightEvents[i], unhighlight, false); } function highlight(e) { dropArea.classList.add('drag-over'); } function unhighlight(e) { dropArea.classList.remove('drag-over'); } dropArea.addEventListener('drop', handleDrop, false); function handleDrop(e) { var dt = e.dataTransfer; var files = dt.files; for (var i = 0; i < files.length; i++) { uploadFile(files[i]); } } function uploadFile(file) { var reader = new FileReader(); reader.onload = function(e) { var data = new Uint8Array(e.target.result); var buffer = Module._malloc(data.length); Module.HEAPU8.set(data, buffer); Module.ccall('emscripten_file_dropped', null, ['string', 'number', 'number'], [file.name, buffer, data.length]); Module._free(buffer); }; reader.readAsArrayBuffer(file); } } }
...
var _emscripten_file_dropped = Module['_emscripten_file_dropped'] = createExportWrapper('emscripten_file_dropped', 3);
var _main = Module['_main'] = createExportWrapper('__main_argc_argv', 2);
var _malloc = Module['_malloc'] = createExportWrapper('malloc', 1);
var _free = Module['_free'] = createExportWrapper('free', 1);
var _TriggerSaveAsCallback = Module['_TriggerSaveAsCallback'] = createExportWrapper('TriggerSaveAsCallback', 2);
var _TriggerSelectFileCallback = Module['_TriggerSelectFileCallback'] = createExportWrapper('TriggerSelectFileCallback', 4);
var _TriggerYesNoCallback = Module['_TriggerYesNoCallback'] = createExportWrapper('TriggerYesNoCallback', 2);
var _AllocateMemory = Module['_AllocateMemory'] = createExportWrapper('AllocateMemory', 1);
var _FreeMemory = Module['_FreeMemory'] = createExportWrapper('FreeMemory', 1);
var _CopyToMemory = Module['_CopyToMemory'] = createExportWrapper('CopyToMemory', 3);
