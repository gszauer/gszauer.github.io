const getTargetHitsToKill = (floor) => {
    if (floor <= 3) return 2;
    if (floor <= 5) return 2;
    if (floor <= 10) return 2.6;
...
const getAttackExpectationBonus = (floor) => {
    if (floor <= 5) return 0;
...
const getFloorHpPadding = (floor) => {
    if (floor <= 5) return 0;
    if (floor <= 10) return 3;
    if (floor <= 15) return 8;
    if (floor <= 20) return 16;
    if (floor <= 25) return 25;
    return 32 + Math.floor((floor - 25) * 3);
};

const getMonsterBaseDamage = (floor) => {
    if (floor <= 3) return 1;
    if (floor <= 5) return 2;
    if (floor <= 10) return 3;
    if (floor <= 15) return 4;
    if (floor <= 20) return 6;
    if (floor <= 25) return 8;
    if (floor <= 30) return 10;
    return 12;
};

const getScaleFactor = (modifier = 1) => {
    return 1 + (modifier - 1) * 0.5;
};
...
class Monster {
    constructor(scene, config, player, key) {
        ...
        // Dynamic Scaling
        const hpMultiplier = Phaser.Math.FloatBetween(0.95, 1.05);
        const hpScale = getScaleFactor(this.scaleModifier);
        const targetHits = getTargetHitsToKill(player.floor);
        const expectedPlayerAttack = Math.max(1, player.getBaseAttack() + getAttackExpectationBonus(player.floor));
        const currentAttack = Math.max(expectedPlayerAttack, player.getAttack());
        const adaptiveAttackBonus = Math.max(0, currentAttack - expectedPlayerAttack) * 0.5;
        const baseHpValue = (expectedPlayerAttack + adaptiveAttackBonus) * targetHits;
        const paddedHp = baseHpValue + getFloorHpPadding(player.floor);
        this.maxHp = Math.max(1, Math.round(paddedHp * hpScale * hpMultiplier));
        this.hp = this.maxHp;

        const attackScale = getScaleFactor(this.scaleModifier);
        const baseDamage = getMonsterBaseDamage(player.floor);
        const adaptiveAttack = Math.max(0, (player.getBaseMaxHp() - 20) * 0.04);
        this.attack = Math.max(1, Math.round((baseDamage + adaptiveAttack) * attackScale));
...
const createSummonedSkeleton = (scene, player) => {
    ...
    const skeletonTargetHits = Math.max(1.5, getTargetHitsToKill(player.floor) * 0.5);
    const skeletonExpectedAttack = Math.max(1, player.getBaseAttack() + getAttackExpectationBonus(player.floor) * 0.5);
    const skeletonHp = skeletonExpectedAttack * skeletonTargetHits + Math.max(2, getFloorHpPadding(player.floor) * 0.3);
    skeleton.maxHp = Math.max(5, Math.round(skeletonHp));
    skeleton.hp = skeleton.maxHp;
    const skeletonBaseDamage = Math.max(1, getMonsterBaseDamage(player.floor) * 0.4);
    const skeletonAdaptive = Math.max(0, (player.getBaseMaxHp() - 20) * 0.02);
    skeleton.attack = Math.max(1, Math.round(skeletonBaseDamage + skeletonAdaptive));
